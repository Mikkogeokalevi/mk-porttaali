...
const LIST_NAME = listNameFromUrl || 'paalista';
let FIREBASE_PATH = LIST_NAME;
let OFFLINE_KEY = `georeissu-offline-${LIST_NAME}`;
// --- ASETUKSET w2 PÄÄTTYVÄT ---

document.addEventListener('DOMContentLoaded', () => {
    document.title = `${LIST_NAME} — MK Reissuapuri —`;
    const listNameDisplay = document.getElementById('listNameDisplay');
    if (listNameDisplay) {
        listNameDisplay.textContent = LIST_NAME;
    }
    ...
    const firebaseConfig = {
        ...
        databaseURL: "https://mk-porttaali-default-rtdb.europe-west1.firebasedatabase.app",
    ...
    const startApp = (uid) => {
        ...
        if (!listNameFromUrl) {
            const allListsRef = ref(database, `reissuapuri/${uid}`);
